<template>
<div ref="root" class="legacy-section-root">
<section>
    <div class="banner">
        <div class="wrap">
            <h2 data-aos="fade-up" data-aos-delay="200" v-html="i18n('index.banner.main.title')"></h2>
            <h3 data-aos="fade-up" data-aos-delay="400" v-html="i18n('index.banner.main.dscpt')"></h3>
        </div>
        <img src="/assets/images/banner/page-index.webp" alt="메인 페이지의 배너 이미지 입니다. AI와 CPU로 형상화된 이미지입니다.">
    </div>
    <div class="banner type-title">
        <div class="wrap">
            <strong class="title" data-aos-delay="800" data-aos="fade-up" v-html="i18n('index.banner.text.title')"></strong>
            <p data-aos="fade-up" data-aos-delay="1000" v-html="i18n('index.banner.text.dscpt')"></p>
        </div>
    </div>
    <div class="banner type-goto">
        <div class="wrap">
            <strong class="title" data-aos="fade-up" data-aos-delay="200" v-html="i18n('index.banner.goto.title')"></strong>
            <a href="/business" data-aos="fade-up" data-aos-delay="400" class="btn type-opacity">
                <span data-aos="fade-up" data-aos-delay="600" v-html="i18n('index.banner.goto.button')"></span>
            </a>
        </div>
        <img src="/assets/images/banner/goto.webp" alt="">
    </div>
    <PartnerLogoSection type="customer" />
    <PartnerLogoSection type="partner" />
    <CommonAskBanner />
</section>
</div>
</template>

<script setup lang="ts">
import { getI18nNamespaces } from 'core/utils/route-i18n'
import { useLegacySectionRoot } from 'core/composables/useLegacySectionRoot'
import CommonAskBanner from '~/components/section/CommonAskBanner.vue'
import PartnerLogoSection from '~/components/section/PartnerLogoSection.vue'

const { i18n } = useLocaleMessage()
const root = ref<HTMLElement | null>(null)
useLegacySectionRoot(root, getI18nNamespaces('/'))
</script>
